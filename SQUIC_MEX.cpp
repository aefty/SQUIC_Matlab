...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    integer *W_j;
    double *W_val;
    integer W_nnz = mxGetNzmax(W);

    if (W_nnz > 0) // M Matrix NOT zeros
    {
        // Copy MATLAB matrix to raw buffer;
    ...
    double *data_test = mxGetPr(prhs[argIdx]);
    integer n_test = mxGetN(prhs[argIdx]);
    integer p_test = mxGetM(prhs[argIdx]);

    if (p_test == 1) //  data_test will not be used
    {
        n_test = 0;
    }
    else if (p_test != 1 && p_test != p) // data_test is not correct
    {
        mexErrMsgIdAndTxt("SQUIC:type",
    ...
    SQUIC_C(
        ...
        max_iter, drop_tol, term_tol, verbose, matrix_format_coo,
        X_i, X_j, X_val, X_nnz,
        W_i, W_j, W_val, W_nnz,
        info_num_iter,
        info_times,
    ...
    if (max_iter == 0) // Special max_iter==0: SQUIC only compute the sparse sample covariance S
    {
        ...
        if (nlhs > argIdx)
        {
            ...
            mwIndex *W_out_i = (mwIndex *)mxGetIr(W_out);
            mwIndex *W_out_j = (mwIndex *)mxGetJc(W_out);
            double *W_out_val = (double *)mxGetPr(W_out);
            ...
            // fix the J vector
            for (integer i = 1; i < p + 1; i++)
            {
                W_out_j[i] = W_out_j[i - 1] + W_out_j[i];
            }
    ...
    else // Regular case return all values
    {
        ...
        mexPrintf("nlhs=%d", nlhs);
        mexPrintf("X_nnz=%d", (mwSize)X_nnz);
        mexPrintf("W_nnz=%d", (mwSize)W_nnz);
        ...
        if (nlhs > argIdx)
        {
            ...
            for (integer i = 1; i < p + 1; i++)
            {
                X_out_j[i] = X_out_j[i - 1] + X_out_j[i];
            }
